...
\usepackage{url}
\usepackage{booktabs}
\title{Embodied Model Repair Memory: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about adaptive world models for long-horizon robot tasks. The original research bet was: Store repairs to model beliefs as reusable state, not transient corrective prompts. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\item Scene Memory Transformer for Embodied Agents in Long-Horizon Tasks (2019)
\item From Real World to Logic and Back: Learning Generalizable Relational Concepts For Long Horizon Robot Planning (2024)
\item MAP-VLA: Memory-Augmented Prompting for Vision-Language-Action Model in Robotic Manipulation (2025)
\item CALVIN: A Benchmark for Language-Conditioned Policy Learning for Long-Horizon Robot Manipulation Tasks (2021)
\end{itemize}
