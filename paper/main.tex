        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Embodied Model Repair Memory: Branch-Structured Evidence for Embodied World-Action Models}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study adaptive world models for long-horizon robot tasks. The motivating bet is: Store repairs to model beliefs as reusable state, not transient corrective prompts.. We introduce a mechanism-level representation that keeps physically admissible but unrealized action outcomes as explicit branches. A synthetic contact-style evaluation shows that the proposed branch mechanism improves hidden-mode success and tail-risk relative to observed-only, augmentation-as-data, and scalar-uncertainty baselines.
        \end{abstract}

        \section{Introduction}
        Robotics papers often collapse unobserved physical alternatives into extra data, variance, or a single predicted next state. For adaptive world models for long-horizon robot tasks, this can erase the difference between modes that are visually similar but action-critical. This paper asks whether a world-action model should expose those alternatives to the planner before execution.
...
observed\_only\_wam & hidden\_mode\_shift & 0.165 & 0.835 \\
augmentation\_as\_data\_wam & nominal & 0.608 & 0.392 \\
augmentation\_as\_data\_wam & hidden\_mode\_shift & 0.233 & 0.767 \\
scalar\_uncertainty\_wam & nominal & 0.685 & 0.315 \\
scalar\_uncertainty\_wam & hidden\_mode\_shift & 0.305 & 0.695 \\
proposed\_branch\_mechanism & nominal & 0.762 & 0.238 \\
proposed\_branch\_mechanism & hidden\_mode\_shift & 0.605 & 0.395 \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        \section{Conclusion}
        Embodied Model Repair Memory argues that embodied world-action models should preserve unrealized physical alternatives when those alternatives change downstream action value.
